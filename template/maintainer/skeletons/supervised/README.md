...
**Contract 侧（不可变）：**
- `prepare_data(cfg)` — 准备数据源
- `test(learner, ws, *, shared_context)` — 官方台账评估
- `should_keep(current_metrics, history_rows)` — keep/discard 决策

**Workspace 侧（可变）：**
- `build_learner(cfg)` — 构建模型
- `build_objective(cfg)` — 构建损失函数
- `train_step(learner, source, objective, *, epoch, shared_context)` — 一轮训练
