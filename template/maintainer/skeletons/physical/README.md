...
**Contract 侧（不可变）：**
- `prepare_data(cfg)` — 准备数据源（physical 通常返回 None）
- `test(learner, ws, *, shared_context)` — 官方台账评估
- `should_keep(current_metrics, history_rows)` — keep/discard 决策

**Workspace 侧（可变）：**
- `build_learner(cfg)` — 构建 PINN 模型
- `build_objective(cfg)` — 构建物理约束损失
- `train_step(learner, source, objective, *, epoch, shared_context)` — 一轮训练（1 stage）
- `evaluate(learner, *, shared_context)` — 训练过程监控评估
- `prepare_data(cfg)` — 准备配制点
